...
type ShortGap = { start_min: number; end_min: number; short_by_min: number };
type W = {
  oven_id: number; oven_label: string;
  start_min: number; end_min: number; duration_min: number;
  ferment_end: number; bake_end: number;
  short_gaps: ShortGap[];
};
...
export default function WindowsPage() {
  ...
  return (<>
    ...
    <tbody>{rows.map(w => <tr key={w.oven_id}>
        ...
        <td className="mono">{w.short_gaps.length
          ? w.short_gaps.map(g => `${fmt(g.start_min)}–${fmt(g.end_min)} 比所需短 ${g.short_by_min} 分钟`).join("；")
          : "—"}</td>
      </tr>)}
      {!rows.length && <tr><td colSpan={6}>无可用窗口</td></tr>}
    </tbody></table>
    <p className="note">搜索范围 08:00–22:00，端点相接的空档可用；没有空档的炉不列出。说明列为更早但短于整段时长的空隙及其起止。</p>
  </>);
}
